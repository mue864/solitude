...
export const FlowIndicator = ({
  ...
  const progress = flow ? Math.min(100, Math.max(0, (currentFlowStep / (totalSteps - 1)) * 100)) : 0;
    ...
    >
      <View className="p-4">
        ...
        <Text className="text-text-secondary text-sm mb-3">
          {sessionType} • {Math.floor(FLOWS[currentFlow as keyof typeof FLOWS][currentFlowStep].duration / 60)} min
        </Text>
        
        <View className="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-2 overflow-hidden mt-2">
          <Animated.View 
